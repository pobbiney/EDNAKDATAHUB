@php 

$pageName = "reports"; 

$subpageName = "reports-assignment";

@endphp

@extends('layouts.app')


@section('content')
<div class="content">
    <div class="content container-fluid">
				
        <!-- Page Header -->
        <div class="page-header">
            <div class="row">
                <div class="col">
                    <h3 class="page-title">Reports</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="/">Dashboard</a></li>
                        <li class="breadcrumb-item active">Reports Manager</li>
                    </ul>
                </div>
            </div>
        </div>
        <!-- /Page Header -->
        
       <!-- Tabs -->
       <section class="comp-section">
        <div class="section-header">
            <h3 class="section-title">Reports Manager</h3>
            <div class="line"></div>
        </div>

        
        <div class="row">
            <div class="col-md-12">
                <div class="card bg-white">
                    <div class="card-header">
                        <h5 class="card-title">Report</h5>
                    </div>
                    <div class="card-body">
                        <ul class="nav nav-tabs nav-tabs-solid">
                             <li class="nav-item"><a class="nav-link active"  href="#solid-tab3" data-bs-toggle="tab">Application Report</a></li>
                        </ul>
                        <div class="tab-content">
                            

                            <div class="tab-pane show active" id="solid-tab3">
                                <div class="row">
                                    <div class="col-xl-12 d-flex">
                                        <div class="card flex-fill">
                                            <div class="card-header">
                                                <h5 class="card-title">Application Report</h5>
                                            </div>
                                            <div class="card-body">
                                                <form id="form" method="POST">
                                                    @csrf
                                                <div class="table-responsive">
                                                    <table align="center"  class="table table-striped table-bordered">
                                                        <tbody>
                                                            <tr>
                                                                <td colspan="4" align="center"><label>Generate</label></td>
                                                            </tr>
                                                            <tr>
                                                                
                                                               
                                                                <td align="center">
                                                                    
                                                                   <b>Region</b>
                                                                   
                                                                </td>
                                                                <td align="center">
                                                                    
                                                                   <b>District</b>
                                                                   
                                                                </td>
                                                                <td align="center">
                                                                    
                                                                   <b>Station</b>
                                                                   
                                                                </td>
                                                                <td></td>
                                                            </tr>
                                                            <tr>
                                                               
                                                                <td>
                                                                   <select class="form-select" name="region" id="region" required>
                                                                        <option value="">-- SELECT REGION --</option>
                                                                        @foreach ($regionList as $regionListItem)
                                                                            <option value="{{ $regionListItem->id }}">{{ $regionListItem->name }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                   <span id="regionerror"></span>
                                                                </td>
                                                                <td>
                                                                <select name="district_id" id="district_id" class="form-select" required>
                                                                    <option value="">-- Select Option --</option>
                                                                </select>
                                                                <span id="district_iderror"></span></td>
                                                                <td>
                                                                    <select class="form-select" name="station" id="station" required>
                                                                        <option value="">-- SELECT STATION --</option>
                                                                        @foreach ($stationsList as $stationsListitem)
                                                                            <option value="{{ $stationsListitem->id }}">{{ $stationsListitem->name }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                   <span id="stationerror"></span>
                                                                </td>
                                                               
                                                                <td><button type="submit" name="search_two" id="search_two" class="btn btn-success btn-sm" ><i class="fa fa-list"></i></button> </td>
                                                            </tr>
                                                        </tbody>    
                                                    </table>
                                                    <br>
                                                    <p align="center" style="display:none; color: limegreen;" id="wait_two"><img src="{{ asset('images/spinner-grey.gif')}}" > Please wait....</p>
                                                    <div id="resultTwo"></div>
                                                </div>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                           
                        </div>
                    </div>
                </div>
            </div>
        </div>
        
    </section>
    <!-- /Tabs -->
        
    
    </div
</div>

@endsection

@section('scripts')




  <script>



    $(document).on('click','#search_two',function(e){
    e.preventDefault();

       
        $("#regionerror").empty();
        $("#district_iderror").empty();
        $("#stationerror").empty();
    
        
        var region =$.trim($("#region").val());
        var district_id = $.trim($("#district_id").val());
        var station  = $.trim($("#station").val());

    if (region.length == 0) {

        $('#regionerror').html('<p><small style="color:red;">field cannot be left empty.</small><p/>');
        $("html, body").animate({ scrollTop: 0 }, "slow");

    }
    if (district_id.length == 0) {

        $('#district_iderror').html('<p><small style="color:red;">field cannot be left empty.</small><p/>');
        $("html, body").animate({ scrollTop: 0 }, "slow");

    }
    if (station.length == 0) {

        $('#stationerror').html('<p><small style="color:red;">field cannot be left empty.</small><p/>');
        $("html, body").animate({ scrollTop: 0 }, "slow");

    }
    if(region.length != 0 && district_id.length != 0 && station.length != 0){

        $('#wait_two').css('display', 'block');

        document.getElementById("search_two").disabled = true;

        $.ajax({
            url:"{{ route('reports-assignment-report-process') }}",
            type:'POST',
            data:{
                "_token": "{{ csrf_token() }}",
                'region':region,
                'district_id':district_id,
                'station':station
            },
            success:function(data){
                
                $('#wait_two').css('display', 'none');

                document.getElementById("search_two").disabled = false;

                if ($.trim(data) == "error") {

                    $("#resultTwo").html('<p class="alert alert-danger" align="center">No Records Found</p>');
                    
                }else{

                    $('#resultTwo').html(data);


                }
            }
        });

    }
});
    </script>

    <script>
    $(document).on("change","#region",function(){
    
    var dropvalue = $("#region").val();
    
    $("#wait_fetch").css("display", "block");
    
    $.ajax({
        type: "POST",
        url: "{{ route('get-district') }}",
        data:{
            "_token": "{{ csrf_token() }}",
            'region_id': dropvalue
        },
        success:function(data) {
    
            $('#district_id').html(data);
           
        }
    
    });
    });
    </script>
@endsection